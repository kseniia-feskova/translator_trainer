...
interface WordDao {
    ...
    suspend fun addWordToAllWordsSet(wordId: String) {
        val allWordsSet = getSetByName(ALL_WORDS) ?: return
        insertSetWordCrossRef(SetWordCrossRef(setId = allWordsSet.id, wordId = wordId))
    }

    suspend fun addWordToSet(wordId: String, setId: String) {
    ...
    suspend fun updateWordStatus(wordId: String, newStatus: WordStatus)
    ...
    @Transaction
    suspend fun deleteWordWithRelations(wordId: String) {
    ...
    suspend fun deleteWordFromSets(wordId: String)
}
